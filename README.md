...
Huddle is a full-stack, real-time video conferencing app built on the MERN stack with Socket.io and WebRTC think of it as a lightweight Google Meet with a built-in collaborative whiteboard, live location sharing, and AI-powered document summaries in chat.
...
- **🎨 Interactive Whiteboard**: Real-time collaborative whiteboard with drawing, undo, redo, adjustable brush size, eraser, and confirmed clear actions, synchronized across all clients, including late joiners.
...
**Backend:** Node.js, Express 5, MongoDB (Mongoose), Socket.io, JWT + bcrypt, Cloudinary + Multer, Joi, Google Gemini API, Jest.
...
2. **Backend Setup**
   ...
   ```
Fill in `.env` with your Mongo URI, JWT secret, Cloudinary credentials, and a `GEMINI_API_KEY` (needed for AI PDF summaries). See `.env.example` for the full list.
...
4. **Docker (alternative)**
   ...
   `docker-compose.yml` at the repo root spins up both services — backend on `:5000` and frontend on `:5173`:
   ```bash
...
- **Nginx** serves the production frontend and reverse-proxies backend requests, while **PM2** manages the Node.js backend process.
...
Both deploy workflows run on a **self-hosted GitHub Actions runner on the AWS EC2 instance** (`runs-on: [self-hosted, huddle]`) rather than GitHub-hosted runners — the job executes directly on the server, pulling `origin/devops`, rebuilding, and restarting the relevant process in place.

There's no separate staging environment; `devops` is the deploy branch. Both jobs target an `ec2-production` GitHub Environment with a required reviewer, so a push to `devops` pauses at "Waiting for review" in the Actions tab until manually approved  the code doesn't reach EC2 until then.